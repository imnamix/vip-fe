...
export default function ServiceEditor({ items, setItems }: Props) {
  ...
  return (
    <div className="space-y-4">
      {items.map((svc, idx) => (
        <div key={svc.id} className="border border-gray-200 dark:border-white/10 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between bg-gray-50 dark:bg-white/5 px-4 py-2.5">
            <span className="font-semibold text-[#212121] dark:text-white text-sm">Service {idx + 1}</span>
            <button onClick={() => remove(svc.id)} className="p-1.5 text-[#D32F2F] hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
              <Trash2 size={13} />
            </button>
          </div>
          <div className="p-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-[#616161] dark:text-gray-400 uppercase tracking-wider mb-1.5">Icon (emoji)</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-[#616161] dark:text-gray-400 uppercase tracking-wider mb-1.5">Title</label>
                <input value={svc.title} onChange={e => update(svc.id, 'title', e.target.value)} placeholder="Service title"
                  className="w-full px-3 py-2 border border-gray-200 dark:border-white/10 rounded-xl text-sm focus:outline-none focus:border-[#D32F2F] bg-white dark:bg-white/5 text-[#212121] dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500" />
              </div>
            </div>
            <div>
              <label className="block text-xs font-semibold text-[#616161] dark:text-gray-400 uppercase tracking-wider mb-1.5">Image URL</label>
              <div className="flex gap-2">
                ...
                <input value={svc.image} onChange={e => update(svc.id, 'image', e.target.value)} placeholder="https://..."
                  className="flex-1 px-3 py-2 border border-gray-200 dark:border-white/10 rounded-xl text-sm focus:outline-none focus:border-[#D32F2F] bg-white dark:bg-white/5 text-[#212121] dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500" />
              </div>
            </div>
            <div>
              <label className="block text-xs font-semibold text-[#616161] dark:text-gray-400 uppercase tracking-wider mb-1.5">Description</label>
