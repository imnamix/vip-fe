...
function VipIcon({ name, size = 22 }: { name: string | null; size?: number }) {
  ...
  return Comp ? (
    <Comp size={size} className="text-white" />
  ) : (
    ...
    <div className={`relative flex-1 mb-4 ${clamped ? "group/desc" : ""}`}>
      ...
      {clamped && (
        <div className="pointer-events-none absolute bottom-full left-0 right-0 z-20 mb-2 hidden group-hover/desc:block">
          <div className="bg-gray-900 text-white text-xs rounded-xl p-3 leading-relaxed shadow-xl">
            {text}
          </div>
        </div>
      )}
...
const benefits = [
  {
    ...
    desc: "Harness number vibrations that magnetically draw financial abundance.",
  },
  {
    icon: Briefcase,
    title: "Career Growth",
    desc: "Align your numbers with your professional path for rapid advancement.",
  },
  {
    icon: Zap,
    title: "Positive Energy",
    desc: "Surround yourself with frequencies that boost vitality and enthusiasm.",
  },
  {
    icon: Award,
    title: "Business Success",
    desc: "Numerologically optimised decisions for sustainable business growth.",
  },
  {
    icon: Heart,
    title: "Better Relationships",
    desc: "Improve harmony in personal and professional relationships.",
  },
  {
    icon: Sparkles,
    title: "Improved Confidence",
    desc: "Numbers that resonate with your core identity build lasting confidence.",
  },
];
...
function ReviewsCarousel({ reviews }: { reviews: Testimonial[] }) {
  ...
  const extended =
    reviews.length > 0
      ? [...reviews, ...reviews.slice(0, Math.min(3, reviews.length))]
      : [];
...
export default function Home() {
  ...
  const [selectedService, setSelectedService] = useState<FetchedService | null>(
    null,
  );
  ...
  const openVipInquiry = (vip: VipNumber) =>
    setInquiryContext({
      lookingFor: `Enquiry for ${vip.vipNumber}`,
      title: vip.vipNumber,
    });

  const openServiceInquiry = (service: FetchedService) =>
    setInquiryContext({
      lookingFor: `Enquiry for ${service.title}`,
      title: service.title,
    });
  ...
  return (
    <div className="overflow-x-hidden">
            ...
            slides.map((slide, i) => (
              ...
              >
                ...
                {(slide.title || slide.description) && (
                  <>
                    <div
                      className={`absolute inset-0 bg-gradient-to-r ${OVERLAYS[i % OVERLAYS.length]}`}
                    />
                      ...
                      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
                        <div className="max-w-2xl">
                          <div className="inline-flex items-center gap-2 bg-[#FBC02D]/20 border border-[#FBC02D]/40 text-[#FBC02D] px-4 py-1.5 rounded-full text-sm font-medium mb-5">
                            <Sparkles size={13} /> India's #1 Numerology
                            Platform
                          </div>
                          <h1
                            className="text-4xl lg:text-6xl font-bold text-white leading-tight mb-5"
          ...
          <section ref={aboutRef} className="py-20 bg-white overflow-hidden">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                ...
                <div
                  className={`relative transition-all duration-700 ease-out ${aboutVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-12"}`}
                >
                  {aboutUs?.homepageAboutUsImage && (
                ...
                <div
                  className={`transition-all duration-700 ease-out delay-150 ${aboutVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-12"}`}
                >
                  <div className="text-[#D32F2F] font-semibold text-xs uppercase tracking-widest mb-3">
                    About VIP Numerology
                  </div>
                  {aboutUs?.homepageAboutUsTitle && (
                    <h2
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                      {aboutUs.statistics.map((s) => (
                        ...
                        >
                          ...
                          <div>
                            <div
                              className="text-[#D32F2F] text-2xl font-bold"
                              style={{ fontFamily: "Poppins, sans-serif" }}
                            >
                              {s.value}
                            </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-14">
              ...
              <p className="text-[#616161] mt-3 max-w-lg mx-auto">
                Exclusive premium numbers curated for maximum positive
                vibrations and life-changing energy.
              </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {vipNumbers.map((n) => (
                <div
                  key={n.id}
                  className="bg-white rounded-2xl p-6 border border-gray-100 hover:border-[#D32F2F]/30 hover:shadow-lg transition-all group relative flex flex-col overflow-hidden"
                >
                  {/* Tag badge */}
                  {n.tag && (
                    <div
                      className={`absolute top-4 right-4 px-2 py-0.5 rounded-full text-xs font-bold ${n.tag === "SOLD OUT" ? "bg-gray-200 text-gray-500" : n.tag === "HOT" ? "bg-[#D32F2F] text-white" : "bg-[#FBC02D] text-black"}`}
                    >
                      {n.tag}
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {n.rating != null && (
                      <div className="ml-3 flex items-center gap-1 text-xs font-semibold text-[#D32F2F] flex-shrink-0">
                        <span>
                          {parseFloat(Number(n.rating).toFixed(1))}/10
                        </span>
                        <Star
                          size={11}
                          className="fill-[#D32F2F] text-[#D32F2F]"
                        />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="inline-block bg-[#FFF8E1] text-[#D32F2F] text-xs font-semibold px-2 py-0.5 rounded-full self-start">
                      {n.category}
                    </div>
                    {n.numerologyScore && (
                      <div className="text-xs font-semibold text-[#616161]">
                        Score: <span className="text-[#212121]">{n.numerologyScore}</span>
                      </div>
                    )}
                  </div>
        ...
        <section className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-14">
              ...
              <p className="text-[#616161] mt-3 max-w-lg mx-auto">
                Comprehensive numerology solutions designed to transform every
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {fetchedServices.slice(0, 6).map((s) => (
                <div
                  key={s.id}
                  className="group rounded-2xl overflow-hidden border border-gray-100 hover:shadow-xl transition-all"
                >
                  ...
                  <div className="p-6">
                    <h3
                      className="font-bold text-[#212121] text-lg mb-2"
                      style={{ fontFamily: "Poppins, sans-serif" }}
                    >
                      {s.title}
                    </h3>
                    <div
                      className="text-[#616161] text-sm leading-relaxed mb-4 line-clamp-2 prose prose-sm max-w-none"
                      dangerouslySetInnerHTML={{ __html: s.description }}
                    />
                    <button
                      onClick={() => setSelectedService(s)}
                      className="text-[#D32F2F] text-sm font-semibold hover:underline flex items-center gap-1"
                    >
        ...
        >
          ...
          >
            <div className="relative h-72 flex-shrink-0">
              ...
              <button
                onClick={() => setSelectedService(null)}
                className="absolute top-4 right-4 w-8 h-8 bg-white/20 border border-white/30 text-white rounded-full flex items-center justify-center hover:bg-white/40 transition-colors"
              >
                <X size={16} />
              </button>
            ...
            <div className="p-6">
              ...
              <button
                onClick={() => {
                  openServiceInquiry(selectedService);
      ...
      <section className="py-20 bg-[#D32F2F]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <div className="text-[#FBC02D] font-semibold text-xs uppercase tracking-widest mb-3">
              Why Choose Us
            </div>
            <h2
            ...
            <p className="text-red-100 mt-3 max-w-lg mx-auto">
              Experience the transformative power of numerology across every
              aspect of your life.
            </p>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-14">
              ...
              <p className="text-[#616161] mt-3">
                Trusted by clients across Pan India.
              </p>
              {avgRating > 0 && (
                <div className="flex items-center justify-center gap-6 mt-6">
                  <div className="text-center">
                    <div
                      className="text-5xl font-bold text-[#D32F2F]"
                      style={{ fontFamily: "Poppins, sans-serif" }}
                    >
                      {avgRating.toFixed(1)}
                    </div>
            ...
            <section className="py-20 bg-[#F5F5F5]">
              <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-12">
                  ...
                  <p className="text-[#616161] mt-3">
                    Everything you need to know about VIP numerology and our
                    services.
                  </p>
                </div>
                  ...
                  {pageFaqs.map((faq) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <span
                            className="font-semibold text-[#212121] text-sm sm:text-base leading-snug"
                            style={{ fontFamily: "Poppins, sans-serif" }}
                          >
                            {faq.question}
                          </span>
      ...
      <section className="py-20 bg-gradient-to-r from-[#212121] to-[#B71C1C]">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-white/80 text-lg mb-8">
            Book Your Personalised Numerology Number
          </p>
