...
const bannerSlides = [
  {
    img: "https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1440&h=400&fit=crop",
    title: "Events & Gallery",
    subtitle: "Upcoming workshops, expos, and our visual showcase.",
  },
  {
    img: "https://images.unsplash.com/photo-1475721027785-f74eccf877e2?w=1440&h=400&fit=crop",
    title: "Join Our Events",
    subtitle:
      "Live masterclasses, seminars, and VIP number expos across India.",
  },
  {
    img: "https://images.unsplash.com/photo-1492538368677-f6e0afe31dcc?w=1440&h=400&fit=crop",
    title: "Our Visual Story",
    subtitle:
      "Explore moments from our events, deliveries, and client journeys.",
  },
];
...
export default function EventsGallery() {
  ...
  const filteredGallery = galleryItems.filter((item) => {
    ...
    if (galleryFilter === "Numerologist")
      return item.category === "numerologist";
            ...
            {eventsLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                {events.map((event) => {
                  ...
                  const badgeClass =
                    STATUS_BADGE[event.eventStatus] ?? "bg-gray-400";
                  const timeRange = (() => {
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="relative h-64 rounded-2xl overflow-hidden mb-8 bg-gray-100">
              {selectedEvent.mainImage?.[0]?.media_url ? (
                <img
                  src={selectedEvent.mainImage[0].media_url}
              ...
              <div
                className={`absolute top-4 left-4 ${STATUS_BADGE[selectedEvent.eventStatus] ?? "bg-gray-400"} text-white text-xs font-bold px-3 py-1 rounded-full`}
              >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8 bg-[#FFF8E1] rounded-2xl p-5">
              {[
                ...
                [
                  ...
                  "Time",
                  (() => {
                    const s = formatTime(selectedEvent.eventTime ?? "");
                    const e = formatTime(selectedEvent.endTime ?? "");
                    return s && e ? `${s} – ${e}` : s || e || "—";
                  })(),
                ],
                ...
                [
                  <Users size={15} />,
                  "Seats",
                  selectedEvent.seats != null
                    ? `${selectedEvent.seats} seats`
                    : "—",
                ],
              ].map(([icon, label, value], i) => (
                <div key={i} className="text-center">
                  <div className="flex justify-center text-[#D32F2F] mb-1">
            ...
            {/* Schedule */}
            {(selectedEvent.schedules ?? []).filter((s) => s.time || s.title)
              .length > 0 && (
              <div className="mb-8">
                <h2
                  className="text-2xl font-bold text-[#212121] mb-5"
                  style={{ fontFamily: "Poppins, sans-serif" }}
                >
                  Schedule
                </h2>
                  ...
                  {(selectedEvent.schedules ?? [])
                    ...
                    .map((s, i, arr) => (
                      <div key={i} className="flex items-center gap-4">
                        <div className="text-[#D32F2F] font-bold text-sm w-24 flex-shrink-0">
            ...
            {/* Event Gallery */}
            {(selectedEvent.galleryImages ?? []).filter((g) => g.media_url)
              .length > 0 && (
              <div className="mb-8">
                <h2
                  className="text-2xl font-bold text-[#212121] mb-4"
                  ...
                  {(selectedEvent.galleryImages ?? [])
                    ...
                    .map((g, i) => (
                      <img
                        ...
                        src={g.media_url}
                        alt=""
                        className="rounded-2xl h-40 object-cover flex-1 min-w-[140px]"
                      />
                    ))}
                </div>
              </div>
                        ...
                        <>
                          ...
                          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-end p-3 opacity-0 group-hover:opacity-100">
                            <div className="bg-white/90 rounded-lg px-3 py-1.5">
                              <div className="text-xs font-medium text-[#212121]">
                                {item.title}
                              </div>
                              <div className="text-xs text-[#D32F2F] capitalize">
                                {item.category}
                              </div>
                            </div>
                          </div>
        ...
        >
          <button
            ...
            onClick={(e) => {
              ...
              setLightbox(
                (l) =>
                  (l! - 1 + filteredGallery.length) % filteredGallery.length,
              );
            }}
          >
            <ChevronLeft size={20} />
          ...
          <div
            className="max-w-4xl max-h-full px-16"
          ...
          >
            ...
            <div className="text-center mt-4">
              <div className="text-white font-medium">
                {filteredGallery[lightbox].title}
              </div>
              <div className="text-[#FBC02D] text-sm capitalize">
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setLightbox((l) => (l! + 1) % filteredGallery.length);
            }}
          >
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-[#D32F2F] text-sm font-medium">
                  {selectedEvent.title}
                </p>
              </div>
              <button
            ...
            <div className="bg-[#FFF8E1] rounded-xl p-4 mb-5 grid grid-cols-2 gap-2 text-sm">
              <div>
                <span className="text-[#616161] text-xs">Date</span>
